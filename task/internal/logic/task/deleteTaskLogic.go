...
func (l *DeleteTaskLogic) DeleteTask(req *types.DeleteTaskRequest) (resp *types.BaseResponse, err error) {
	...
	if l.svcCtx.UnifiedEmailService != nil && len(emails) > 0 {
		taskData := map[string]interface{}{
			...
			"message":      fmt.Sprintf("任务 %s 已被 %s 删除", taskInfo.TaskTitle, operatorName),
		}

		if err := l.svcCtx.UnifiedEmailService.SendTaskNotification(l.ctx, "task_deleted", emails, taskData); err != nil {
			l.Logger.WithContext(l.ctx).Errorf("发送任务删除邮件失败: %v", err)
		}
	}
